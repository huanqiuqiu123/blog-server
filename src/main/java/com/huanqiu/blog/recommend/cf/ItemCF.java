...
public class ItemCF {
    ...
    public static List<Result> computeNeighbourByItem(String userId, Map<String, List<RelateEntity>> map) {
        List<Result> resultList = new ArrayList<>();
        List<RelateEntity> userItemList = map.get(userId);
        map.forEach((k, v) -> {
            if (!k.equals(userId)) {
                double dist = computeDistByItem(userItemList, v);
    ...
    private static double computeDistByItem(List<RelateEntity> xList, List<RelateEntity> yList) {
